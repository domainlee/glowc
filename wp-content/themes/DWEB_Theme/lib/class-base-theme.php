...
abstract class BB_Theme {
    ...
    protected function add_taxonomy( $args, $post_type ) {
        $labels = array(
            ...
            'search_items'      => 'Tìm kiếm ' .  $args['plural'],
            'all_items'         => 'Tất cả ' .  $args['plural'],
            'parent_item'       => 'Cha ' . $args['singular'],
            'parent_item_colon' => 'Cha ' . $args['singular'] . ':',
            'edit_item'         => 'Sửa ' . $args['singular'],
            'update_item'       => 'Cập nhật ' . $args['singular'],
            'add_new_item'      => 'Thêm mới ' . $args['singular'],
            'new_item_name'     => 'Mới ' . $args['singular'] . ' Name',
    ...
    protected function add_post_type( $args ) {
        $labels = array(
            ...
            'add_new'            => 'Thêm mới',
            'add_new_item'       => 'Thêm mới ' . $args['singular'],
            'new_item'           => 'Mới ' . $args['singular'],
            'edit_item'          => 'Sửa ' . $args['singular'],
            'view_item'          => 'Xem ' . $args['singular'],
            'all_items'          => 'Tất cả ' . $args['plural'],
            'search_items'       => 'Tìm kiếm ' .  $args['plural'],
            'parent_item_colon'  => 'Cha ' . $args['plural'] . ':',
        ...
        $post_type = array(
            'labels'             => $labels,
            'description'        => __( 'Mô tả.', self::$text_domain ),
            'public'             => true,
...
//			'has_archive'        => $args['has_archive'] ? $args['has_archive'] : false,
            'has_archive'        => true,
            'hierarchical'       => false,
            'menu_position'      => $args['menu_position'] ? $args['menu_position']:null,
            'menu_icon'          => $args['icon'],
